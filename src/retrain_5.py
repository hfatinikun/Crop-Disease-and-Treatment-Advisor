...
class WeightedCrossEntropyLoss(nn.Module):
    """
    CrossEntropyLoss that scales each sample's loss by a per-sample weight.
    Used to down-weight pseudo-mask samples (weight=0.5) vs real masks (weight=1.0).
    """
    ...
    def forward(
        ...
        # Scale each image's loss by its sample weight
        sample_weights = sample_weights.to(logits.device).float()
        weighted = ce * sample_weights.view(-1, 1, 1)   # broadcast over H, W
